...
from beer.util.misc import format_bytes
from beer.util.misc import format_minutes
from beer.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total beer farmed: {amounts['farmed_amount'] / units['beer']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['beer']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['beer']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'beer start wallet' and 'beer wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'beer wallet show'")
    else:
        print("Note: log into your key using 'beer wallet show' to see rewards for each key")
