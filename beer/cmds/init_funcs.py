...
from beer.util.ints import uint32
from beer.util.keychain import Keychain
from beer.util.path import mkdir
from beer.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'beer keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xbr_target_address" not in config["farmer"]
    stop_searching_for_pool = "xbr_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xbr_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xbr_target_address"):
    ...
    if "xbr_target_address" not in config["farmer"]:
        ...
        print(
            ...
            f"{config['farmer']['xbr_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xbr_target_address" not in config["pool"]:
        print(f"Setting the xbr destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["xbr_target_address"] = all_targets[0]
    elif config["pool"]["xbr_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xbr_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    beer_ca_crt, beer_ca_key = get_beer_ca_crt_key()
    beer_ca_crt_path = ca_dir / "beer_ca.crt"
    beer_ca_key_path = ca_dir / "beer_ca.key"
    beer_ca_crt_path.write_bytes(beer_ca_crt)
    beer_ca_key_path.write_bytes(beer_ca_key)
    ...
    beer_ca_crt, beer_ca_key = get_beer_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, beer_ca_crt, beer_ca_key, False, overwrite=False)
...
def beer_version_number() -> Tuple[str, str, str, str]:
...
def beer_minor_release_number():
    res = int(beer_version_number()[2])
    print(f"Install release number: {res}")
    return res


def beer_full_version_str() -> str:
    major, minor, patch, dev = beer_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def beer_init(root_path: Path):
    if os.environ.get("BEER_ROOT", None) is not None:
        print(
            f"warning, your BEER_ROOT is set to {os.environ['BEER_ROOT']}. "
            f"Please unset the environment variable and run beer init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Beer directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if BEER_ROOT is set, or if user has run beer init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'beer keys show --show-mnemonic-seed'")
