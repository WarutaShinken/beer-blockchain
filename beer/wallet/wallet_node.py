...
from beer.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from beer.protocols import wallet_protocol
from beer.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from beer.protocols.protocol_message_types import ProtocolMessageTypes
from beer.protocols.wallet_protocol import (
...
from beer.server.node_discovery import WalletPeers
...
from beer.util.errors import Err, ValidationError
from beer.util.ints import uint32, uint128
from beer.util.keychain import Keychain
from beer.util.lru_cache import LRUCache
from beer.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from beer.wallet.settings.settings_objects import BackupInitialized
...
from beer.wallet.wallet_blockchain import ReceiveBlockResult
from beer.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'beer keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSBeerConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSBeerConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSBeerConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSBeerConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSBeerConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSBeerConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
