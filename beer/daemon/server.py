...
from beer.cmds.init_funcs import beer_init
from beer.daemon.windows_signal import kill
from beer.server.server import ssl_context_for_root, ssl_context_for_server
from beer.ssl.create_ssl import get_mozilla_ca_crt
from beer.util.beer_logging import initialize_logging
from beer.util.config import load_config
from beer.util.json_util import dict_to_json_str
from beer.util.path import mkdir
from beer.util.service_groups import validate_service
from beer.util.setproctitle import setproctitle
from beer.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Beer.")
    quit()
...
service_plotter = "beer plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "beer": "beer",
        "beer_wallet": "start_wallet",
        "beer_full_node": "start_full_node",
        "beer_harvester": "start_harvester",
        "beer_farmer": "start_farmer",
        "beer_introducer": "start_introducer",
        "beer_timelord": "start_timelord",
        "beer_timelord_launcher": "timelord_launcher",
        "beer_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("beer daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered BEER_ROOT
    os.environ["BEER_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with BEER_ROOT: {os.environ['BEER_ROOT']}")
