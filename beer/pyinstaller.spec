...
ROOT = pathlib.Path(importlib.import_module("beer").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `beer` file name (which is the executable)
    and the `beer` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `beer` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("beer-blockchain"))[0]
...
# TODO: collapse all these entry points into one `beer_exec` entrypoint that accepts the server as a parameter

entry_points = ["beer.cmds.beer"] + [f"beer.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "beer.util.bip39"])

if THIS_IS_WINDOWS:
    beer_mod = importlib.import_module("beer")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/beer/util/english.txt", "beer/util"))
datas.append((f"{ROOT}/beer/util/initial-config.yaml", "beer/util"))
datas.append((f"{ROOT}/beer/wallet/puzzles/*.hex", "beer/wallet/puzzles"))
datas.append((f"{ROOT}/beer/ssl/*", "beer/ssl"))
...
add_binary("beer", f"{ROOT}/beer/cmds/beer.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/beer/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/beer/server/start_{server}.py", COLLECT_ARGS)
