...
from beer.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from beer.full_node.full_node_api import FullNodeAPI
from beer.server.start_farmer import service_kwargs_for_farmer
from beer.server.start_full_node import service_kwargs_for_full_node
from beer.server.start_harvester import service_kwargs_for_harvester
from beer.server.start_introducer import service_kwargs_for_introducer
from beer.server.start_service import Service
from beer.server.start_timelord import service_kwargs_for_timelord
from beer.server.start_wallet import service_kwargs_for_wallet
from beer.simulator.start_simulator import service_kwargs_for_full_node_simulator
from beer.timelord.timelord_launcher import kill_processes, spawn_process
from beer.types.peer_info import PeerInfo
from beer.util.bech32m import encode_puzzle_hash
from beer.util.block_tools import BlockTools, test_constants
...
async def setup_farmer(
    ...
    config["xbr_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xbr")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xbr_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xbr")
